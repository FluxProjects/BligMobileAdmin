...
<li class="d-inline-block px-2"><b>Add</b></li>
</ul>
<section class="section_body">
  <form method="post" enctype="multipart/form-data" action="{{ url('product/insert') }}" >
  	{{ csrf_field() }}
	<div class="border rounded bg-light px-4 pb-5">
		...
		<div class="row">
			<div class="col-sm-3 position-relative">
				...
				<input type="text" class="form-control border-blue" placeholder="Product Name" name="P_name" required>
			</div>
			<div class="col-sm-3 position-relative _category">
				<i class="fas pr-3 fa-th c_icon green"></i>
				<select onchange="_appendSubCat(this)" class="form-control border-green" required placeholder="Product Name" name="P_cat">
			...
			<div class="col-sm-3 position-relative">
				...
				<input type="number" class="form-control border-orange" placeholder="Delivery Time" name="delivery_time" required>
			</div>
			<div class="col-sm-3 position-relative P_aprox">
				<i class="fas fa-dollar-sign c_icon pink"></i>
				<input type="text" class="form-control border-purple" placeholder="Price Approx" name="P_aprox" required>
			</div>
			<div class="col-sm-3 mt-4 position-relative">
				<i class="fab fa-stack-overflow c_icon orange"></i>
				<input type="number" class="form-control border-orange" placeholder="Min Prices" name="mun_p" required>
			</div>
		</div>
		...
		<div class="details">
			<div class="row">
				<div class="col-sm-3 position-relative">
					...
					<input type="text" class="form-control border-green" placeholder="Label" name="lable[]" required>
				</div>
				<div class="col-sm-3 position-relative">
					<i class="fab fa-product-hunt c_icon pink"></i>
					<input type="text" class="form-control border-purple" placeholder="Value" name="value[]" required>
				</div>
				<i onclick="_append()" class=" fas fa-plus blue"></i>
			</div>
		</div>
		...
		<div class="discreption">
			<div class="row">
				<div class="col-sm-6 position-relative">
					<i class="fab fa-product-hunt c_icon green"></i>
					<textarea class="form-control border-green" name="discription" required></textarea>
				</div>
			</div>
		</div>
...
@section('jquery')
<script type="text/javascript">
	function _appendSubCat(cat_id) {
		$('.P_aprox').removeClass('mt-4');
		$.ajax({
			url: '{{ url("sub-admin") }}/'+$(cat_id).val(),
			type: 'get',
			dataType: 'json',
		})
		.done(function(resp) {
			$('.P_aprox').addClass('mt-4');
			$('._Subcategory').remove();
			var _option='';
			$(resp).each(function(i,val) {
				_option+='<option value='+val.id+'>'+val.sub_category_name+'</option>'
			});

			$('._category').after('<div class="col-sm-3 position-relative _Subcategory"><i class="fas pr-3 fa-th c_icon green"></i><select class="form-control border-green" placeholder="Product Name" name="P_scat"><option disabled selected>Select Sub-Category</option>'+_option+'</select></div>')
		});
